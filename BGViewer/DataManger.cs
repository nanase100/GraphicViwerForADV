	 ...
	 
{
	...
	public class GenreTree
	{
		...
		public GenreTree()
		{
			...
			m_parentGenre	    = null;
			m_treenodeColor     = Color.FromArgb(0,0,0);
			m_useBigThumbnail   = 0;
		}
	...
	public class DataManger
	{
		...
		public int                                  m_historyLeft			{ set; get; }
		public int                                  m_historyTop			{ set; get; }
		public int                                  m_historyWidth			{ set; get; }
		public int                                  m_historyHeight			{ set; get; }
		public int									m_historyPosType		{ set; get; }
		...
		public void SettingSave(string settingFilePath)
		{
			...
			jsonData.履歴ウインドウ座標[0]	= m_historyLeft;
			jsonData.履歴ウインドウ座標[1]	= m_historyTop;
			jsonData.履歴ウインドウ座標[2]	= m_historyWidth;	
			jsonData.履歴ウインドウ座標[3]	= m_historyHeight;	
			jsonData.履歴ウインドウ接続位置	= m_historyPosType;
		...
		public void Load(string settingFilePath,string graphicDirectoryPath)
		{
			...
			Regex		regLine6					= new Regex( "^\t*([^\r\n\"]*?),[ \t]*([^\r\n\"]*?),[ \t]*([^\r\n\"]*?),([^\r\n\"]*?),([^\r\n\"]*?),([^\r\n\"]+?),[ \t]*\"([^\r\n\"]*)\"");			//主にユーリス用ファイル名ではなく、テキストを置き換え文に使用
					...
					while (true)
					{
						...
						if ( matchResult.Success == true || matchResult2.Success == true || matchResult3.Success == true || matchResult4.Success == true )
						{
							...
							if (m_genreMaster.Add(nowTotalGenre) == true)
							{
								...
								if (matchType == 1)
								{
									tmpGenreItme = new GenreTree(nowGenre, nowTotalGenreSplit, paerentGenreItem, R, G, B, isAutoExpand, int.Parse(matchResult.Groups[10].Value));
								}
								else if( matchType == 3 )
								{
									tmpGenreItme = new GenreTree(nowGenre, nowTotalGenreSplit, paerentGenreItem, R, G, B, isAutoExpand, int.Parse(matchResult.Groups[10].Value), (matchResult.Groups[11].Value == "1"?true:false) );
								}
								else if (matchType == 4)
								{
									tmpGenreItme = new GenreTree(nowGenre, nowTotalGenreSplit, paerentGenreItem, R, G, B);
								}
								else
								{
									tmpGenreItme = new GenreTree(nowGenre, nowTotalGenreSplit, paerentGenreItem, R, G, B, isAutoExpand);
								}
            ...
            {
				MessageBox.Show("フォルダ構造の開閉を示す「※」と「_※」の個数が一致しませんでした。graphic.txtをの「※」と「_※」の個数を確認してください。");
			}
		}
	}
		...
		public class readJsonType1  {
			...
			public List<int>	履歴ウインドウ座標		{ get; set; } = new List<int>();
			public int			履歴ウインドウ接続位置	{ get; set; }
